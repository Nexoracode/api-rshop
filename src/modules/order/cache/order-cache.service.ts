...
export class OrderCacheService {
    ...
    constructor(
        ...
        this.logger = new Logger(OrderCacheService.name);
    }
     ...
     */
    async clearAlOrderCache(): Promise<void> {
        try {
            ...
            this.logger.log(`✅ ${deletedCount} کلید order پاک شد`);
        } catch (error) {
    ...
    async getCacheStats(): Promise<{
        ...
        try {
            ...
            return {
                ...
                adminListKeys: cleanKeys.filter(k => k.includes('admin:list:')).length,
                userListKeys: cleanKeys.filter(k => k.includes('user:') && k.includes(':list:')).length,
                detailKeys: cleanKeys.filter(k => k.includes('detail:')).length,
                statsKeys: cleanKeys.filter(k => k.includes('stats:')).length,
            };
        } catch (error) {
